...
/*
	* checks the ASCII value of a character to determine whether it is a printable character or not. 
	*/
...
/*
	...
	*The ft_strlen() function returns an integer with the length of the string 
	*excluding the NULL character. The ft_strlen() function counts the alphabets, whitespaces, special symbols, and numbers 
	*until it encounters the NULL character in the string.
	*/
char	*ft_strchr(const char *str, int c);
/*
	*returns a pointer to the first occurrence of c that is converted to a character in string. The function returns NULL if the specified character is not found.
	*/
char	*ft_strrchr(const char *str, int c);
/*
	*function finds the last occurrence of c (converted to a character) in string . 
	*The ending null character is considered part of the string . The strrchr() function returns a pointer to the last occurrence of c in string . 
	*If the given character is not found, a NULL pointer is returned.
	*/
int		ft_strcmp(const char *s1, const char *s2);
/*
	*Its primary purpose is to compare the characters of the two strings in sequence until it finds a mismatch 
	*or until the end of the strings is reached (that is, the null character '\0').
	*/
size_t	ft_strlcat(char *dest, const char *src, size_t size);
/*
	*The ft_strlcat() function takes the same arguments as ft_strcat() ,
	*but it also includes the full size of the destination buffer as an additional argument. 
	*This allows ft_strlcat() to ensure that no buffer overflow occurs. 
	*It's an excellent alternative to ft_strcat() when the size of the destination buffer is known.
	*/
char	*ft_strnstr(const char *s, const char *s2, size_t n);
/*
	*locates the first occurrence of the null-terminated string \c little in the string \c big, where not more than \c len characters are searched
	*/
size_t	ft_strlcpy(char *dest, const char *src, size_t size);
/*
	*returns the total length of the string that would have been copied if there was unlimited space
	*/
char    *ft_strdup(const char *src);
/*
	*returns a pointer to a null-terminated byte string, which is a duplicate of the string pointed to by s. 
	*The memory obtained is done dynamically using malloc it can be freed using free(). 
...
/*
	...
	*It returns an integer less than, equal to, or greater than zero according to whether s1 is lexicographically less than, equal to, or greater than s2.
	*/
void	*ft_memchr(const void *s, int c, size_t n);
/*
	*searches the first count bytes of buf for the first occurrence of c converted to an unsigned character. The search continues until it finds c or examines count bytes.
	*/
...
/*
	*used to copy data from a memory location to another memory location. 
	*The function uses pointers to represent the address in which the data are stored. 
	*The ft_memcpy function returns a pointer that stores the address of the destination after the copy has been performed.
	*/
